...
void STKTexManager::reloadAllTextures(bool mesh_texture_only)
{
    ...
    if (mesh_texture_only)
    {
        ...
        for (auto d : *TrackManager::get()->getAllTrackDirs())
        {
            if (!d.empty() && d.back() == '/')
                d.pop_back();
            mesh_texture_paths.insert(fs->getAbsolutePath(
        ...
        for (unsigned i = 0; i < TrackManager::get()->getNumberOfTracks(); i++)
        {
            io::path sc = TrackManager::get()->getTrack(i)->getScreenshotFile()
                .c_str();
            if (!sc.empty())
